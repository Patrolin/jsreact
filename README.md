...
```
then clicking the button works correctly, but pressing Enter causes the form to be submitted with an old value.

There are 3 different ways to fix this:
...
3) Use a direct reference to the state instead, like:
 ```tsx
    ...
    // ...
    const [state, changeState] = useChangeState({username: ""});
    <form onSubmit={() => console.log("submit", state.username)}>
